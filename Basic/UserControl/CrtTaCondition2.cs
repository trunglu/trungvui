...
namespace GP8000.UserControl
{
	...
	/// </summary>
	public class CrtTaCondition2 : System.Windows.Forms.UserControl
	{
		...
		public System.Windows.Forms.RadioButton radChucVu;
		/// <summary> 
		/// Required designer variable.
		/// </summary>
		private System.ComponentModel.Container components = null;

		public CrtTaCondition2()
		{
		...
		public string GetWhere(string table,Boolean vacate)
		{
			...
			if(tb!="")
				tb=tb+".";
			if((R_NON.Checked || radChucVu.Checked)&& !R_ALL.Checked)
			{
		...
		private void CrtTaCondition2_Load(object sender, System.EventArgs e)
		{
		...
		private void radChucVu_CheckedChanged(object sender, System.EventArgs e)
		{
			Radio_Change();
		}
		...
		{
			bool isAll = R_ALL.Checked == true ? true: false;			
			
			string result="";
			if(!isAll)
			{				
				string strField = ck.Checked == false ? "EMP_ID":"EMP_I1";
				strField = radChucVu.Checked == false ? strField:"POS_ID";
				if(R_DEP.Checked)	//bo phan								
					result = string.Format("WHERE bophan.DEP_NM = N'{0}'", CB_DEP.Text.ToString());				
				else if(R_WID.Checked)	// ma
				{
					string from = txt_fr.Text;
					string to= txt_to.Text;										
					result = string.Format("WHERE nhanvien.{0} Between N'{1}' and N'{2}'",strField, from,to);
				}
				else if(R_NON.Checked || radChucVu.Checked)	// khong lien tuc // chuc vu
				{
					string arrayList="";
					for (int i=0;i<list.Count;i++)
					{
