...
namespace GP8000.Reports
{
	...
	public class frmTaLabourContract : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.txt_LAB_NM.TextChanged += new System.EventHandler(this.txt_LAB_NM_TextChanged);
			// 
			// label2
			// 
...
//						{
//							DateTime dateTime1 = Convert.ToDateTime(vs4.Rows[i]["LAB_ST"]);
//							dateTime1=dateTime1.AddDays(+0);
//							vs4.Rows[i]["LAB_ST"]=dateTime1;
//						}
//					}
//					else
//					{
					if(vs4.Rows[i]["LAB_ED"]!=null)
					{
						DateTime dateTime1 = Convert.ToDateTime(vs4.Rows[i]["LAB_ED"]);
						dateTime1=dateTime1.AddDays(-1);
						vs4.Rows[i]["LAB_ED"]=dateTime1;
						//					}
...
//			Tab_3_print();
		...
		private void Tab_3_print()
		{
			...
			if(ck_14.Checked)
			{
				for(int i=1;i<vs4.Rows.Count;i++)
				{
                    ...
                    PublicFunction.SQL_Execute(sql);
					//tu cap nhat BH khi ki HD Chinh thuc cho LeeShin
					if(PublicFunction.CUS_ID=="44" && vs4.Rows[i]["LAB_NO"]+""=="HDLD1")
					{
						sql="UPDATE FILB01A SET BHX_BT=1 WHERE EMP_ID='"+vs4.Rows[i]["EMP_ID"]+"'";
						PublicFunction.SQL_Execute(sql);
					}
				}
			}
		...
		private void nav5_TBarClick(object sender, ToolBarButtonClickEventArgs e)
		{
			...
			{
				string temp=vs5.Rows[vs5.Row]["EMP_ID"].ToString();
				if(vs5.Row>0)
				{
					...
					if(max==vs5.Rows[vs5.Row]["SEQ_N2"]+"")
					{
						...
						else
						{
							if(rs.rows==0)
							{
								string kdel="delete from FILB01AF where EMP_ID=N'"+temp+"'" ;
								PublicFunction.SQL_Execute(kdel);
							}
		...
		private void txt_LAB_NM_TextChanged(object sender, System.EventArgs e)
		{
