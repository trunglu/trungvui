...
//			CheckCols();		
		...
		{			
			...
			for (int i=vs.Selection.r1;i<=vs.Selection.r2;i++)
			{				
				sql="UPDATE FILC06A SET USR_CK='"+ vs.Rows[e.Row]["USR_CK"] +"',LOC_B1='"+ tam +"'";
				sql=sql+" where EMP_ID=N'"+vs.Rows[i]["EMP_ID"]+"' and ATT_DT='"+ DateTime.Parse( vs.Rows[i]["ATT_DT"].ToString()).ToString ("yyyy/MM/dd")+"'";
				PublicFunction.SQL_Execute(sql);
			}
