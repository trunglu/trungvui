...
namespace GP8000.Attendance
{
	...
	/// </summary>
	public class frmTaMealFormula : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.nav1.Location = new System.Drawing.Point(56, 0);
			...
			this.Name = "frmTaMealFormula";
			this.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen;
			this.Text = "frmTaMealFormula";
			this.Load += new System.EventHandler(this.frmTaMealFormula_Load);
		...
		#endregion
		#region init
		private void frmTaMealFormula_Load(object sender, System.EventArgs e)
		{
			Init_Form();
...
//				vs1.Rows[vs1.Rows.Count-1]["DAT_TP"]=rs.record(i,"DAT_TP");
		...
		private void vs_DoubleClick(object sender, EventArgs e)
		{
			...
			{
				if(txt1!=null )
					txt1.Text="["+vs.Rows[vs.Row]["MEAL_ID"]+"]";				
			}
		}
...
//			dlg1.ShowDialog();
		...
		{								
			...
			{	
				...
				vs2.Rows[vs2.Rows.Count-1]["SEQ_NO"]=T_String.GetMax("MAX(SEQ_NO)","FILC15B")+"";
...
//			{	
			...
			{				
				try
				{
					...
					if(item=="["+Item+"]")
						item_type=1;								
					vl=vl.Replace("["+Item+"]","(FLOOR(["+Item+"]/100)+(["+Item+"]/100-FLOOR(["+Item+"]/100))/0.6)");					
					con=con.Replace("["+Item+"]","(FLOOR(["+Item+"]/100)+(["+Item+"]/100-FLOOR(["+Item+"]/100))/0.6)");											
				}
				catch{}
			}
			...
			string sql="Update FILC15A SET"+item+"="+vl+" from"
				+" FILB01A b, FILC06A a, FILC15A c where ( a.EMP_ID=b.EMP_ID and b.EMP_ID=c.EMP_ID)";
			if(con+""!="")
				sql+= " and " + con;
			return sql;
		}
