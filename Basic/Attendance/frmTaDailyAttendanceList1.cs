...
//			CheckCols();		
		...
		{			
			...
			{	
				...
				sql="UPDATE FILC06A SET USR_CK='"+ vs.Rows[e.Row]["USR_CK"] +"',LOC_B1='"+ tam +"' where EMP_ID=N'"+vs.Rows[i]["EMP_ID"]+"' and ATT_DT='" + ngay.ToString("yyyy/MM/dd") + "'";
				PublicFunction.SQL_Execute(sql);
			}
			//			string sql="";
			//			int tam=0;
...
//					for(int i=1;i<vs.Rows.Count;i++)
//					{
//						if (Convert.ToString(vs.Rows[e.Row]["USR_CK"])!="")
//							if(Convert.ToString(vs.Rows[e.Row]["USR_CK"])!=PublicFunction.A_UserID)
//							{
//								e.Cancel=true;
//							}
//							else
...
//					}
					...
					if(vs.Cols[e.Col].Name=="LOC_B1" && e.Row>0)
					{
						for(int i=1;i<vs.Rows.Count;i++)
						{
							if (vs.Rows[e.Row]["USR_CK"]+""!="" && PublicFunction.A_UserID!= vs.Rows[e.Row]["USR_CK"]+"")
							{
								e.Cancel=true;
							...
							{
								string test=vs.Rows[e.Row]["USR_CK"]+"";
								if (vs.Rows[e.Row]["USR_CK"]+""=="" && PublicFunction.A_UserID!= vs.Rows[e.Row]["USR_CK"]+"")
								{
									e.Cancel=true;
...
*/
		...
		private void vs_AfterEdit(object sender, C1.Win.C1FlexGrid.RowColEventArgs e)
		{
			...
			for (int i=vs.Selection.r1;i<=vs.Selection.r2;i++)
			{
				...
				ngay=(DateTime)(vs.Rows[i]["ATT_DT"]);
				sql="UPDATE FILC06A SET USR_CK='"+ vs.Rows[e.Row]["USR_CK"] +"',LOC_B1='"+ tam +"' where EMP_ID=N'"+vs.Rows[i]["EMP_ID"]+"' and ATT_DT='" + 
