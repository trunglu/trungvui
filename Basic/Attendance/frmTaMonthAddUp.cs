...
namespace GP8000.Attendance
{
	...
	public class frmTaMonthAddUp : System.Windows.Forms.Form
	{
		...
		{  
			...
			//ShowMonthAddUps(dt1.Text,dt2.Text);
			Init_Form();
			Set_w();
			Init_Menu();
			Set_LB();
		...
		{			
			...
			nav.cmd_delete.Visible=nav.cmd_add.Visible=nav.cmd_save.Visible=nav.cmd_search.Visible=false;									
...
//				txt.Text=vs.Cols[txt.Tag+""].Caption;			
		...
		private void txt_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e)
		{
			if(e.KeyCode==Keys.Enter)
			{
				...
				if(ck.Checked)
				{
					sql=sql+" and  (DEL_BT=0 or DEL_BT is null)  ";
					if(PublicFunction.GetOption("MONTHADDUP")!="1")
						sql+=" and (YYY_MM between N'"+dt1.Text+"' and N'"+dt2.Text+"')";
...
*/
		...
		private void vs_AfterEdit(object sender, C1.Win.C1FlexGrid.RowColEventArgs e)
		{
			...
			for (int i=vs.Selection.r1;i<=vs.Selection.r2;i++)
			{
				...
				//vs.Rows[e.Row]["LST_DT"]=T_String.GetDate().ToString("yyyy/MM/dd HH:mm");
				sql=string.Format("UPDATE {0} SET LST_NM='",Function.Common.Ints.tbTongKet)+ vs.Rows[e.Row]["LST_NM"] +"',LST_DT='"+T_String.GetDate().ToString("yyyy/MM/dd HH:mm")+"',LOC_B1='"+ tam +"' where EMP_ID=N'"+vs.Rows[i]["EMP_ID"]+"' and YYY_MM=N'"+vs.Rows[i]["YYY_MM"]+"'";
				PublicFunction.SQL_Execute(sql);
			}
		}	
		////////////////////////////////////////////////////////
