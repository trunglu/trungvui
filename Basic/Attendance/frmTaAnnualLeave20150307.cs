...
namespace GP8000.Attendance
{
	...
	public class frmTaAnnualLeave : System.Windows.Forms.Form
	{
		...
		{			
			...
			//pn cho quang viet
			if(PublicFunction.GetOption("TYP_BT")=="1")
			{								
				vs.Cols["TYP_HIS"].ComboList="|...";
				vs.Cols["TYP_HIS"].Style.ForeColor=Color.HotPink;
			}
...
//			nav.Show_VS("DEP_ID=N'"+Dep.Get_Data("DEP_ID")+"' and (VAC_BT=0 or VAC_BT is null) and YYY_YY=N'"+ dt.Text+"'");				
			...
			{					
				...
				else
				{
					if (optWorking.Checked==true)
						...
						sql=sql+" and VAC_BT=1";
				}				
				sql=sql+" and YYY_YY=N'"+ dt.Text+"' and DEP_ID in (Select DEP_ID  from SYS_SECURITY_DEP"+
					" where MNU_ID="+this.Tag+" and ALL_BT=1 and USER_ID=N'"+PublicFunction.A_UserID+"') order by a.EMP_ID";									
				nav.Show_VS(sql);	
				for(int i=1;i<vs.Rows.Count;i++)
				{
					if(vs.Rows[i]["VAC_BT"]+""=="True")
		...
		{
			frmTaCalAnnualLeave dlg=new frmTaCalAnnualLeave( DateTime.Parse( dt.Value+"") );
			dlg.Tag=this.Tag;
			dlg.ShowDialog();
		}
		...
		{
			frmTaViewNatureSwitching dlg=new frmTaViewNatureSwitching(vs.Rows[e.Row]["EMP_ID"]+"",DateTime.Parse( dt.Value+""));
			if( dlg.ShowDialog()==DialogResult.OK)
			{
				//vs.Rows[e.Row][e.Col]=dlg.st;
			}
		}
	}
}
