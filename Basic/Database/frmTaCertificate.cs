...
namespace GP8000.Database
{
	...
	/// </summary>
	public class frmTaCertificate : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.vs.Name = "vs";
			this.vs.Size = new System.Drawing.Size(460, 224);
			...
			this.Name = "frmTaCertificate";
			this.Text = "frmTaCertificate";
			this.Load += new System.EventHandler(this.frmTaCertificate_Load);
		...
		private void frmTaCertificate_Load(object sender, System.EventArgs e)
		{
		...
		{					
			if (e.Button.Equals(nav.cmd_add))
			{
				if (vs.Rows.Count==1 || (int)vs.Rows[vs.Rows.Count-1].UserData!=2)					
					nav.ToolBar_Click(e);									
				vs.Rows[vs.Rows.Count-1]["CTF_ID"]=T_String.GetMax("MAX(CTF_ID)","FILA10B").ToString("000");
				return;
			}
			if (e.Button.Equals(nav.cmd_delete ))
			{
				if( T_String.IsNullTo0(T_String.GetDataFromSQL("Count(EMP_ID)","FILB08A",vs.Cols[2].Name+"=N'"+vs.Rows[vs.Row ][2]+"' "))>0)
				{
