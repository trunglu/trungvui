...
namespace GP8000.Database
{
	...
	/// </summary>
	public class frmTaPosition : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.Name = "frmTaPosition";
			this.Text = "frmTaPosition";
			this.Load += new System.EventHandler(this.frmTaPosition_Load);
		...
		private void frmTaPosition_Load(object sender, System.EventArgs e)
		{
		...
		{					
			if (e.Button.Equals(nav.cmd_add))
			{
				if (vs.Rows.Count==1 || (int)vs.Rows[vs.Rows.Count-1].UserData!=2)					
					nav.ToolBar_Click(e);									
				vs.Rows[vs.Rows.Count-1]["POS_ID"]=T_String.GetMax("MAX(cast( POS_ID AS int))","FILA07A").ToString("000");
				return;
			}
