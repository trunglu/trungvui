...
{
	/// <summary>
	/// Summary description for frmTaGroupType.
	/// </summary>
	public class frmTaGroupType : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.Name = "frmTaGroupType";
			this.Text = "frmTaGroupType";
			this.Load += new System.EventHandler(this.frmTaGroupType_Load);
		...
		private void frmTaGroupType_Load(object sender, System.EventArgs e)
		{
			...
			string sql="Select GRP_ID,GRP_NM from FILA08A";	
		...
		{					
			if (e.Button.Equals(nav.cmd_add))
			{
				if (vs.Rows.Count==1 || (int)vs.Rows[vs.Rows.Count-1].UserData!=2)					
					nav.ToolBar_Click(e);									
				vs.Rows[vs.Rows.Count-1]["GRT_ID"]=T_String.GetMax("MAX(GRT_ID)","FILA09A")+"";
				return;
			}
