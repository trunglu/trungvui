...
{
	/// <summary>
	/// Summary description for frmTaRace.
	/// </summary>
	public class frmTaRace : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.panel3.Size = new System.Drawing.Size(728, 36);
			...
			this.vs.Name = "vs";
			this.vs.Size = new System.Drawing.Size(720, 308);
			...
			this.Text = "frmTaRace";
			this.Load += new System.EventHandler(this.frmTaRace_Load);
		...
		{					
			if (e.Button.Equals(nav.cmd_add))
			{
				if (vs.Rows.Count==1 || (int)vs.Rows[vs.Rows.Count-1].UserData!=2)					
					nav.ToolBar_Click(e);									
				vs.Rows[vs.Rows.Count-1]["TYP_ID"]=T_String.GetMax("MAX(TYP_ID)","FILA11A").ToString("000");
				return;
			}
		...
		private void vs_AfterEdit(object sender, C1.Win.C1FlexGrid.RowColEventArgs e)
		{
			...
			{
				vs.Rows[e.Row]["BLT_NM"]=PublicFunction.A_UserID;
				vs.Rows[e.Row]["BLT_DT"]=T_String.GetDate().ToString("yyyy/MM/dd HH:mm");
			}
			if(!a)
			{
		...
		private void frmTaRace_Closed(object sender, EventArgs e)
		{
			a=true;
		}
	}
