...
//				nav.Show_VS("a.EMP_ID=N'"+wh+"'");							
		...
		private void Init_vs()
		{
			...
			nav.sql+=string.Format(" from {0} a Left JOIN FILB01A b on a.EMP_ID=b.EMP_ID  Left JOIN FILB01AC c on a.EMP_ID=c.EMP_ID",Function.Common.Ints.tbLuongTD);
			nav.tb=Function.Common.Ints.tbLuongTD;
...
//					Show_vs2();Show_vs3();
		...
		{				
			...
			vs2.Visible=false;
			string sql=string.Format("Select * from {0} where EMP_ID=N'",Function.Common.Ints.tbLuongTD)+vs1.Rows[vs1.Row]["EMP_ID"]+"' and YYY_MM=N'"+
				dt.Text+"' and (SEQ_NO=1)";
			Func.RecordSet rs=new Func.RecordSet(sql,PublicFunction.C_con);
			sql=string.Format("Select * from {0} where EMP_ID=N'",Function.Common.Ints.tbLuongTD)+vs1.Rows[vs1.Row]["EMP_ID"]+"' and YYY_MM=N'"+
		...
		{	
			...
			vs3.Visible=false;
			string sql=string.Format("Select * from {0} where EMP_ID=N'",Function.Common.Ints.tbTongKet)+vs1.Rows[vs1.Row]["EMP_ID"]+"' and YYY_MM=N'"+
				dt.Text+"' and (SEQ_NO=1)";
			Func.RecordSet rs=new Func.RecordSet(sql,PublicFunction.C_con);
			sql=string.Format("Select * from {0} where EMP_ID=N'",Function.Common.Ints.tbTongKet)+vs1.Rows[vs1.Row]["EMP_ID"]+"' and YYY_MM=N'"+
				dt.Text+"' and (SEQ_NO=2)";
			Func.RecordSet rs1=new Func.RecordSet(sql,PublicFunction.C_con);
...
//				vs3.Rows[vs3.Rows.Count-1]["ITM_FD"]= rs.Field(i);
		...
		{		
			...
			if (Dep.vs1.Row>0)
				 sql+=" and a.DEP_ID in "+Dep.Get_DepAll()+""; 
...
//		}
		...
		{
			if(vs.Rows[e.Row]["LCK_BT"]+""=="True")
			{
				if(PublicFunction.A_UserID!= vs.Rows[e.Row]["LST_NM"]+"" || vs.Cols[e.Col].Name!="LCK_BT")
					e.Cancel=true;
			}
			else
			{
				if(vs.Cols[e.Col].Name!="LCK_BT" && vs.Tag+""=="")
				{
					//e.Cancel=true;
...
//			}
		...
		private void vs_AfterEdit(object sender, C1.Win.C1FlexGrid.RowColEventArgs e)
		{
			for (int i=vs.Selection.r1;i<=vs.Selection.r2;i++)
			{
				string user=vs.Rows[i]["LST_NM"]+"";
				if (vs.Rows[i]["LST_NM"]+""==PublicFunction.A_UserID || vs.Rows[i]["LST_NM"]+""=="")
				{
				}
				else
				{
			...
			if(vs.Cols[e.Col].Name=="LCK_BT" && e.Row>0)
			{
				if(vs.Rows[e.Row]["LCK_BT"]+""=="True")
				{
				...
				{
					vs.Rows[e.Row]["LST_NM"]="";
					vs.Rows[e.Row]["LST_DT"]="";
				}
			}
			string sql="";
			int tam=0;
			...
			for (int i=vs.Selection.r1;i<=vs.Selection.r2;i++)
			{
				...
				//ngay=T_String.GetDate().ToString("yyyy/MM/dd");
				//vs.Rows[e.Row]["LST_DT"]=T_String.GetDate().ToString("yyyy/MM/dd HH:mm");
				sql=string.Format("UPDATE {0} SET LST_NM='",Function.Common.Ints.tbLuongTD)+ vs.Rows[e.Row]["LST_NM"] +"',LST_DT='"+T_String.GetDate().ToString("yyyy/MM/dd HH:mm")+"',LCK_BT='"+ tam +"' where EMP_ID=N'"+vs.Rows[i]["EMP_ID"]+"' and YYY_MM=N'"+vs.Rows[i]["YYY_MM"]+"'";
		...
		public static void InsertData(string EMP_ID,SqlConnection con,string chuoiketnoi,DateTime fromdate,DateTime todate,String dept)
		{
			...
			{
				string salary=string.Format("select * from {0} Where EMP_ID='",Function.Common.Ints.tbLuongTD)+ EMP_ID +"' AND SEQ_NO=2 AND YYY_MM = '"+fromdate.ToString("yyyyMM") +"'";
				Func.RecordSet rssalary=new Func.RecordSet(salary,PublicFunction.C_con);
				if (rssalary.rows==0)
				{
					string insertsalary=string.Format("insert into {0}(EMP_ID,YYY_MM,SEQ_NO,DEP_ID) values ('",Function.Common.Ints.tbLuongTD)+ EMP_ID +"','"+fromdate.ToString("yyyyMM")+"','2','"+ dept +"')";
