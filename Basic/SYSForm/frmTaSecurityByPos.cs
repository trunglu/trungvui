...
{
	/// <summary>
	/// Summary description for frmTaSecurityByPos.
	/// </summary>
	public class frmTaSecurityByPos : System.Windows.Forms.Form
	{
		...
		public frmTaSecurityByPos(string GROUP_ID,string MNU_ID,string label)
		{
		...
		{			
			...
			sql+="Select N'"+USER_ID+"' as USER_ID,TYP_ID,"+MNU_ID+" as MNU_ID,0 as ALL_BT"+
			...
			{	
				...
				for(int i=1;i<vs1.Rows.Count;i++)
				{
					if(vs1.Rows[i]["ALL_BT"]+""=="True")
						sql="update SYS_SECURITY_TYP set ALL_BT=1 where USER_ID=N'"+USER_ID+"'  and TYP_ID=N'"+
							vs1.Rows[i]["TYP_ID"]+"' and MNU_ID="+MNU_ID;
					else
						sql="update SYS_SECURITY_TYP set ALL_BT=0 where USER_ID=N'"+USER_ID+"'  and TYP_ID=N'"+
							vs1.Rows[i]["TYP_ID"]+"' and MNU_ID="+MNU_ID;
					PublicFunction.SQL_Execute(sql);
				}
				edit=false;
