...
namespace Basic.Function
{
	...
	public class PublicFunction
	{
		...
		public static SqlConnection C_con,C_condel,C_conGGM;
		...
		public static string CUS_ID,L_Lag1="",GGM_server,GGM_database,GGM_username,GGM_password;
		...
		public static Boolean DEP_Show_ALL=true;
		...
		public static void Init_System()
		{
			...
			//{
			GGM_database=GetOption("GGMDATABASE");
			GGM_server=GetOption("GGMSERVER");
			GGM_username=GetOption("GGMUSER");
			GGM_password=GetOption("GGMPASS");
			//}
			openfilekey();
			Get_Note();
			connection();
			...
			process=T_String.IsNullTo0(GetOption1("PROCESS")+"");
			if(process!=1)
				process=2;
		...
		public static void connection()
		{
			...
			if(PublicFunction.GetOption("GGM_BT")=="1")
				C_conGGM=connect(GGM_server,GGM_database,GGM_username,GGM_password);
			if (DT_databasedel!="")
...
//		}
		...
		public static string GetOption1(string note)
		{
			string fn;
			fn=Path+"//option.ini";
			return F_GetNote1("["+note+"]",fn);
		}	
		#endregion
		#region  Get_Config		
		...
		public static void  savefilekey1(string filename,string database)
		{
			StreamWriter f;			
			try
			{
				...
				f.WriteLine(code(DT_server,"DT_server"));
				f.WriteLine(code(database,"DT_database"));
				f.WriteLine(code(DT_username,"DT_username"));
				f.WriteLine(code(DT_password,"DT_password"));
				f.WriteLine(code(AD_Key,"AD_Key"));
				f.WriteLine(code(DT_databasedel,"DT_databasedel"));
		...
		public static string F_GetNote1(string note,string FileName)//lay du lieu tu file config
		{
			try
			{
				string st;
...
//		}
		...
		static public void InitNav_dlg(System.Windows.Forms.Form fr,C1.Win.C1FlexGrid.C1FlexGrid vs,Navigator1._0.NavigatorAuto nav1)
		{
			nav1.Grid=vs;	
			nav1.FormName=fr.Name+"_" + vs.Name   ;			
			nav1.Connection=PublicFunction.C_con;
...
//			return a;
//		}
		public static IDictionary InitCBForVS(string sql)
		{
			return InitCBForVS(sql, true);
		}

		public static IDictionary InitCBForVS(string sql, bool empty)
		{
			int i;
			...
			for (i=0; i<rs.rows; i++)
			{
				ht.Add(rs.record(i,0), rs.record(i,1));
			}
			SortedList sortHash = new SortedList(ht, Comparer.Default);
			ht = sortHash;
		...
		public static void InitCB(ComboBox cb, string sql,Boolean empty)
		{
			...
			for(int i=0;i<rs.rows;i++)
			{
				...
				tb1.Rows.Add(a);
				//tinh chieu rong
				int newWidth = (int)g.MeasureString(rs.record(i, 1)+"", font).Width + 10 + vertScrollBarWidth;
		...
		{
			string sql = "IF EXISTS(SELECT TOP 1 1 FROM " + table + (where == "" ? "" : " WHERE " + where) + ") SELECT 1 ELSE SELECT 0";
