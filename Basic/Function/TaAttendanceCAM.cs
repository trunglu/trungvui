...
namespace GP8000.Function
{
	...
	public class TaAttendance
	{
		...
		public double LAT_MN=0,LATMN1=0,LATMN2=0;
		public int LAT_TM=0;
		public double EAR_MN=0,EARMN1,EARMN2=0;
		public int EAR_TM=0;
        public double LAM_MN = 0;//TINH PHUT TRE GIO AN COM
        public double EAM_MN = 0;//TINH PHUT SOM GIO AN COM
...
//				sql="Update FILC06B set ";
            ...
            if (PublicFunction.GetOption("EARLY_MEAL") == "1")
            {
                //them tre, som gio an com
                sql += ",LAM_MN=N'" + LAM_MN + "'";
                sql += ",EAM_MN=N'" + EAM_MN + "'";
                //end
            }
...
//				AbsentToVacate();					
		...
		public Boolean CheckPhep(double t1,Func.RecordSet phep,int j)
		{
...
//			sql="Select *,LEA_NM from FILC04A a,FILA15A b where a.LEA_ID=b.LEA_ID and EMP_ID=N'"
//				+EMP_ID+"' and STR_DT<='"+ATT_DT+"' and END_DT>='"+ATT_DT+"'";
//			Func.RecordSet phep1=new Func.RecordSet(sql,con);
//			sql="Select * from FILC05A where EMP_ID=N'"
//				+EMP_ID+"' and STR_DT<='"+ATT_DT+"' and END_DT>='"+ATT_DT+"'";
//			Func.RecordSet OutTrip1=new Func.RecordSet(sql,con);
			...
			{				
				double t1=IsN(ATT[j]+"");
                //Thu them tre som giua gio
                double t1Af = 0;
                double t1Bf = 0;
                if (j > 0) t1Bf = IsN(ATT[j - 1] + "");
                if (j < ATT.Count - 1) t1Af = IsN(ATT[j + 1] + "");
                //end
				if(t1!=0)
				{
...
//					if(rsca.record(i,"TYP_ID")=="ATT_HR" || rsca.record(i,"TYP_ID")=="NIG_HR")
//					{		
					//string st=rsca.record(i,"LAT_BT")+"";
                    if (rsca.record(i, "LAT_BT") + "" == "True" || (rsca.record(i, "TYP_ID") == "ATT_HR" || rsca.record(i, "TYP_ID") == "NIG_HR" || rsca.record(i, "TYP_ID") == "NIG_OT"))
					{
						if(j%2==0 && t1<c1)// Late
						{							
							break;
							...
							if(T_String.CongTG(c1,ONN_MN)<=t1 )							
							{
								if(!CheckPhep(t1,phep,j) && !(CheckPhep(c1,phep,j) && (min!=0 && t1==min)) )
								{
                                    if (!CheckPhep(t1, OutTrip, j) && !(CheckPhep(c1, OutTrip, j) && (min != 0 && t1 == min)))
                                    {
                                        ...
                                        LAT_TM++;
                                        //TACH THEM 2 DOAN TRE
                                        if (LAT_TM == 1) LATMN1 = LT;
                                        if (LAT_TM == 2) LATMN2 = LT;
                                        //tach them cot tre luc an com
                                        if (PublicFunction.GetOption("EARLY_MEAL") == "1")
                                        {
                                            if (rsca.record(i, "LAM_BT") + "" == "True")
                                                LAM_MN = LT;
                                        }
                                        //end
                                    }
							...
							if(T_String.CongTG(t1,OFF_MN)<=c2 )
							{
								...
								if(!CheckPhep(t1,phep,j))
								{
									if(!CheckPhep(t1,OutTrip,j))
									{
										...
										EAR_TM++;
                                        //TACH THEM 2 DOAN SOM
                                        if (EAR_TM == 1) EARMN1 = EA;
                                        if (EAR_TM == 2) EARMN2 = EA;
                                        //tach them cot som luc an com
                                        if (PublicFunction.GetOption("EARLY_MEAL") == "1")
                                        {
                                            if (rsca.record(i, "EAM_BT") + "" == "True")
                                                EAM_MN = EA;
                                        }
                                        //end
										//}
...
//			}
		...
		{	
			...
			for(int i=0;i<rsca.rows;i++)
			{
				...
				if(rsca.record(i,"TYP_ID")=="NIG_HR" || rsca.record(i,"TYP_ID")=="NIG_OT")
				{
					if(MAX_HR1>0 && MAX_HR1<T_String.TruTG(c2,c1))
